...
public class TpccStatements {
    ...
    public TpccStatements(Connection conn, int fetchSize) throws Exception {
        ...
        // NewOrder statements.
        pStmts[0]  = prepareStatement("SELECT c.c_discount, c.c_last, c.c_credit, w.w_tax FROM bmsql_customer AS c JOIN bmsql_warehouse AS w ON c.c_w_id = w_id AND w.w_id = ? AND c.c_w_id = ? AND c.c_d_id = ? AND c.c_id = ?");
        pStmts[1]  = prepareStatement("SELECT d_next_o_id, d_tax FROM bmsql_district WHERE d_id = ? AND d_w_id = ? FOR UPDATE");
        pStmts[2]  = prepareStatement("UPDATE bmsql_district SET d_next_o_id = ? + 1 WHERE d_id = ? AND d_w_id = ?");
        pStmts[3]  = prepareStatement("INSERT INTO bmsql_oorder (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES(?, ?, ?, ?, ?, ?, ?)");
        pStmts[4]  = prepareStatement("INSERT INTO bmsql_new_order (no_o_id, no_d_id, no_w_id) VALUES (?,?,?)");
        pStmts[5]  = prepareStatement("SELECT i_price, i_name, i_data FROM bmsql_item WHERE i_id = ?");
        pStmts[6]  = prepareStatement("SELECT s_quantity, s_data, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10 FROM bmsql_stock WHERE s_i_id = ? AND s_w_id = ? FOR UPDATE");
        pStmts[7]  = prepareStatement("UPDATE bmsql_stock SET s_quantity = ? WHERE s_i_id = ? AND s_w_id = ?");
        pStmts[8]  = prepareStatement("INSERT INTO bmsql_order_line (ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
